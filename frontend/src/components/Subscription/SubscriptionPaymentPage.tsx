...
export function SubscriptionPaymentPage() {
    ...
    useEffect(() => {
        ...
        async function loadCheckout() {
            ...
            try {
                const response = await fetch(`${API_URL}/api/payments/subscription/checkout/${checkoutId}`, {
                    headers: {
    ...
    useEffect(() => {
        ...
        async function syncStatus() {
            ...
            try {
                const response = await fetch(`${API_URL}/api/payments/subscription/checkout/${checkoutId}/sync`, {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                ...
                if (payload.status === 'COMPLETED') {
                    setStatusMessage('Payment completed successfully. Your Pro access is now active.');
                ...
                router.replace(`/dashboard/subscription/payment?checkoutId=${checkoutId}`);
            } catch (error) {
                if (isMounted) {
        ...
        return (
            <div className={styles.page}>
                <div className={styles.shell}>
                    <div className={styles.noticeError}>No checkout session was provided. Please go back and start the checkout again.</div>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className={styles.page}>
                <div className={styles.shell}>
                    ...
                    <button type="button" className={styles.backButton} onClick={() => router.push('/dashboard/subscription')}>
                        Back to Checkout
                    </button>
                </div>
            </div>
            ...
            <div className={styles.shell}>
                <div className={styles.headingBlock}>
                    ...
                    <p className={styles.pageSubtitle}>
                        Securely finish your {checkout.summary.productName} purchase using PayU hosted checkout.
                    </p>
                </div>
                            ...
                            <div className={styles.gatewayWrap}>
                                ...
                                {checkout.status === 'COMPLETED' ? (
                                    <div className={styles.summaryMeta}>
                                        Payment confirmed via PayU.
                                        {checkout.payu.mihpayid && <span> Reference: {checkout.payu.mihpayid}</span>}
                                        {checkout.payu.bankRefNum && <span> - Bank Ref: {checkout.payu.bankRefNum}</span>}
                                    </div>
                                ) : (
                                    <>
                        ...
                        <div className={styles.summaryCard}>
                            <div className={styles.summaryTop}>
                                ...
                                <span className={styles.originalPrice}>
                                    {formatCurrency(checkout.summary.originalAmount, checkout.summary.currency)}
                                </span>
                            </div>

                            <div className={styles.summaryLine}>
                                <div>
                                    <p className={styles.productName}>{checkout.summary.productName}</p>
                                    <p className={styles.productMeta}>Billed Annually</p>
                                </div>
                                <span className={styles.summaryPrice}>
                                    {formatCurrency(checkout.summary.discountedAmount, checkout.summary.currency)}
                                </span>
                            </div>
                            ...
                            <div className={styles.taxLine}>
                                <span>GST ({checkout.summary.taxPercent}%)</span>
                                <span>{formatCurrency(checkout.summary.taxAmount, checkout.summary.currency)}</span>
                            </div>
                            ...
                            <div className={styles.summaryMeta}>
                                Billing details and PayU transaction data are saved against your user account after the payment completes.
                            </div>
                            ...
                            {checkout.status !== 'COMPLETED' && (
                                <Link href="/dashboard/subscription" className={styles.receiptLink}>
